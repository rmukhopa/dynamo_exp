...
Large Language Models (LLMs) in various configurations using vLLM.
...
> **Note**: For a non-dockerized deployment, first export `DYNAMO_HOME` to point to the dynamo repository root, e.g. `export DYNAMO_HOME=$(pwd)`

#### Aggregated serving
```bash
cd $DYNAMO_HOME/examples/llm
dynamo serve graphs.vllm.agg:vLLMApiServer -f ./graphs/vllm/configs/agg.yaml
```

#### Aggregated serving with KV Routing
```bash
cd $DYNAMO_HOME/examples/llm
dynamo serve graphs.vllm.agg_router:vLLMApiServer -f ./graphs/vllm/configs/agg_router.yaml
```

#### Disaggregated serving
```bash
cd $DYNAMO_HOME/examples/llm
dynamo serve graphs.vllm.disagg:vLLMApiServer -f ./graphs/vllm/configs/disagg.yaml
```

#### Disaggregated serving with KV Routing
```bash
cd $DYNAMO_HOME/examples/llm
dynamo serve graphs.vllm.disagg_router:vLLMApiServer -f ./graphs/vllm/configs/disagg_router.yaml
```
...
You must have first followed the instructions in [deploy/dynamo/helm/README.md](../../../deploy/dynamo/helm/README.md)
to install Dynamo Cloud on your Kubernetes cluster.

> **Note**: Note the `KUBE_NS` variable in the following steps must match the Kubernetes namespace
> where you installed Dynamo Cloud. You must also expose the `dynamo-store` service externally.
...
> **Note**: For instructions on building and pushing the Dynamo base image,
> see the [Building the Dynamo Base Image](../../../README.md#building-the-dynamo-base-image) section in the main README.
...
DYNAMO_TAG=$(dynamo build graphs.vllm.agg:Frontend | grep "Successfully built" |  awk '{ print $NF }' | sed 's/\.$//')
```
...
dynamo deployment create $DYNAMO_TAG --no-wait -n $DEPLOYMENT_NAME -f ./graphs/vllm/configs/agg.yaml
```
...
Once you create the Dynamo deployment, a pod prefixed with `yatai-dynamonim-image-builder` will begin running.
Once it finishes running, pods will be created using the image that was built.
Once the pods prefixed with `$DEPLOYMENT_NAME` are up and running, you can test out your example!
