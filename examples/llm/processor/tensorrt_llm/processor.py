...
from .utils.chat_processor import ChatProcessorMixin
from .utils.parser import parse_tensorrt_llm_args
from .utils.protocol import DynamoTRTLLMChatCompletionRequest
...
class Processor(BaseProcessor, ChatProcessorMixin):
    worker_class = TensorRTLLMWorker
    router_class = Router
    ...
    def __init__(
        ...
        self.min_workers = 1
        self.args = args
        ChatProcessorMixin.__init__(self, engine_config)

    # --------------Interface implementation--------------#
    @async_on_start
    async def async_init(self):
        await super().async_init(router_mode=self.args.router)
